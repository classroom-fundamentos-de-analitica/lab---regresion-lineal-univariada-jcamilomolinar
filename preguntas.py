...
def pregunta_03():
    ...
    X_fertility = df["fertility"].copy().values.reshape((len(df["fertility"]), 1))

    # Asigne a la variable los valores de la columna `life`
    y_life = df["life"].copy().values.reshape((len(df["life"]), 1))
    ...
    prediction_space = np.linspace(
        min(X_fertility),
        max(X_fertility),
    ).reshape(len(np.linspace(min(X_fertility), max(X_fertility))), 1)
